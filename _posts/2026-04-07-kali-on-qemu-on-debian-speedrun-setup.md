...
After years of virtualbox and vmware, I finally moved to qemu. I run debian as my daily driver, and qemu/kvm is *native* to the kernel, so you can forget about the third-party kernel modules that break on every update or the _"please reinstall guest additions"_ rituals. 🤮
...
It doesn't work out of the box, so here's the short version that actually works.
...
Boot it up. Default credentials are `kali` / `kali`[^3].
...
[^3]: if you did not already know that reconsider your career choices.
