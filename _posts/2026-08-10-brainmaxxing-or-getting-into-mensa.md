---
layout: post
slug: brainmaxxing-or-the-art-of-failing-better
title: brainmaxxing or the art of failing better
tags: [misc]
---
...
Mensa is this organization for people with high IQs. To enter you need to take a in-person test and score in the top 2% (98th percentile), which is two-sigmas away from the mean.

It started in the 40s in the UK and started in Spain around the 80s. Since then there are tons of replications in different countries. Their mission can be summarized as:
1. identify and promote human intelligence for the benefit of humanity.
...
Years later I did start studying at UNED, but a BSc in philosophy (plot twist!). There I managed to get several honors while working full-time, plus doing some freelance collabs on the side. I was particularly fascinated by the analytical stream: logic, philosophy of science, etc. I was living in the UK at the time. That was an intense period of my life, but I remember it with some nostalgia.

In order to prioritize my career, I put the degree on hold[^5]. I hadn't taken it up with a career focus in mind, for obvious reasons. However, I think it had an indirect positive impact on my career in terms of structured reasoning, writing, creativity, etc.

Then I pivoted from development to cybersec, then to offsec specifically, and [my linkedin](https://www.linkedin.com/in/mllamazares/) tells the rest of the story. Along the way I failed miserably (still do, but with style 😎) at tons of projects, but they somehow contributed to who I am today. For instance, years ago I built an algotrading setup, and it was a nice excuse to learn statistics and probability, as well as how to design and implement high-frequency, data-intensive systems[^6].
...
First, my older son. He is currently ~4 years old, and I think he is showing early signs of giftedness: extraordinary memory, a complex sense of humor, musicality, the ability to focus for long periods of time, etc. But well, I’m his father, so I’m probably super-biased. I believe there are more parents of gifted kids than actual gifted kids, kek. At least it was enough to make me question it, since there is a decent heritability factor.
...
I joined Mensa for the following reasons:
- **networking**: we are trained with the same dataset. And as soon as you get older, you're somewhat limited in making new connections. Here the premise is quite interesting: you meet people you share something in common with (high IQ), but they can be virtually of any background, profession, etc. I find that very enriching. 
- **conferences and events**: they organize recurring conferences that are quite interesting. [Most of them are uploaded to YT](https://www.youtube.com/user/mensaes). Low-key, I'd been reviewing them for a while before actually joining. 
- **SIGs (Special Interest Groups)**: a fancy term for forums that gather people with shared interests, but for some reason it sounds cooler here. They also organize meetups every now and then.
- **occasional discounts**: I noticed there are some discounts for members, for instance on board games.
...
Having a nice set of tools is the equivalent of IQ for a carpenter. But if he doesn't put in the effort, take risks, make mistakes, and so on, he won't become a good carpenter. On the other hand, someone with only a spoon and grit might still end up creating awesome pieces of woodcraft. I think this analogy is pretty visual.
...
So effort and intelligence are necessary but not sufficient conditions for success. We need to focus on objective achievements (what you've done that sets you apart). We don't remember Einstein because he was a smart guy, but because of his specific contributions to science—and for sticking his tongue out in those funny pictures.

When I interview a candidate for a tech role, I put more weight on verifiable things you've done (conferences, GitHub repos, bug bounties, certifications, blogs, etc.) than on years of experience or standard education. I want proof of work that reflects passion and shows you can actually solve problems.

## fluid vs crystalized intelligence

There are two types of intelligence: fluid 💧 and crystalized 💎.

Fluid intelligence is the raw problem-solving horsepower. It shoots up fast, hits its personal best around age 23, and then starts sliding downhill like my will to live after the third meeting of the day.

Crystallized intelligence, on the other hand, is all the stuff you've actually learned and stored. It climbs more slowly, peaks later at around age 35, and then declines at a gentler pace.

Here's a cool chart made by my mate claude to illustrate both concepts over a lifespan:
...
/* ---------------------------------------------------------------------------
   ...
   Parameters below are verbatim from that paper's Table 8. Evaluating them
   reproduces the peak ages reported in its Table 9 (Gf 22.8, Gc ~35).
   Plotted as percent of each ability's own lifetime peak, with 0 = the model's
   value at birth — a valid affine transform of the Rasch W scale.
--------------------------------------------------------------------------- */

(function () {
  const MODEL = {
    gf: { mu0: -116.5, mu1: 156.3, Bb: 0.0052, Ba: 0.1539 },
    gc: { mu0: -116.3, mu1: 179.2, Bb: 0.0026, Ba: 0.1104 }
  };

  const W    = (t, p) => p.mu0 + p.mu1 * (Math.exp(-p.Bb * t) - Math.exp(-p.Ba * t));
  const peak = p => Math.log(p.Ba / p.Bb) / (p.Ba - p.Bb);

  function series(p) {
    const base = W(0, p), span = W(peak(p), p) - base, out = [];
    for (let t = 0; t <= 90; t += 0.5) out.push({ x: t, y: (W(t, p) - base) / span * 100 });
    return out;
  }

  const PEAK_GF = peak(MODEL.gf);   // 22.8
  const PEAK_GC = peak(MODEL.gc);   // 34.8
  ...
  const peakMarkers = {
    ...
    afterDatasetsDraw(chart) {
      ...
      const marks = [
        ...
        { age: PEAK_GC, color: ORANGE, label: 'Gc peak \u00B7 35', align: 'left'  }
      ];
      ctx.save();
      marks.forEach(m => {
        ...
        ctx.fillText(m.label, m.align === 'right' ? px - 6 : px + 6, py - 10);
      });
      ctx.restore();
    }
  };
  ...
  new Chart(document.getElementById('intelligenceChart'), {
    ...
    data: {
      datasets: [
        line('Fluid (Gf)', series(MODEL.gf), BLUE),
        line('Crystallized (Gc)', series(MODEL.gc), ORANGE)
      ]
    },
    options: {
      responsive: true,
...
In my case, that blue curve peaked a decade ago and there's nothing I can do about it. The orange one keeps climbing for another 30 years and that part is almost entirely on me. So the only variable I still control is the boring one: showing up and accumulating stuff.
...
[^8]: Curves are generated from the published dual-exponential latent growth model in McArdle, Ferrer-Caja, Hamagami & Woodcock (2002), ["Comparative longitudinal structural analyses of the growth and decline of multiple intellectual abilities over the life span," Developmental Psychology 38(1), 115–142.](http://www.iapsych.com/wj3ewok/LinkedDocuments/McArdle2002.pdf) Woodcock–Johnson–R, N ≈ 1,200, ages 2–95.
